...
8. **Two known overlap warnings are expected.** `battle/Armament.nmm` and `battle/Class.nmm` each have a 1-byte set-reference dropdown overlapping a 2-byte padding field at offset `0x00`. The converted set must validate with **zero errors and exactly these two warnings** — this is the golden assertion for the validator.
...
11. **JSON5 numeric handling.** Serde + the `json5` crate naturally rejects strings, floats-into-ints, negatives-into-uints, and `Infinity`/`NaN`. It parses `+5` as `5`, which the spec explicitly permits. Enforce everything else post-parse: integral, finite, range, and sign requirements.
